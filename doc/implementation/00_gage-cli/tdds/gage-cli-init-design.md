...
*Eight of the nine `D-ENROLL-*` decisions below are resolved.
`D-ENROLL-SEAL-COST` is open: half of it is a settled requirement not to
drop an existing safeguard on a new decryption path, and half is a
security-parameter choice awaiting review.* Everything
...
### `[ ]` D-ENROLL-SEAL-COST — what opening a request is allowed to cost

**Two halves. The first is settled and is a straight bug to avoid; the
second is a genuine parameter choice and is left open for review.**

Raised because "the load-bearing invariant: `pending/` is inert" was
overclaiming. Inertness governs authority, not cost, and every input to
the cost is attacker-controlled: how many files sit in `pending/`, and
what work factor each one's scrypt stanza *claims*.

**Settled: the open path caps the claimed work factor, exactly as the
...
**Open: what work factor gage *writes* a seal at, and whether the number
of requests one run attempts is bounded.**
...
Two candidate answers, and they compose rather than compete:

- **Seal at a much lower factor**, licensed explicitly by the generated
  entropy. This removes the cost problem at its root and makes batch
  approval fast. The coupling has to be recorded where someone would
  break it: **the low factor is licensed by the code being generated and
  80 bits.** If a user-chosen or shorter code is ever allowed
  (D-ENROLL-CODE-FORMAT's deferred word-list option keeps the entropy,
  so it is fine), the factor has to go back up in the same change.
- **Bound how many requests one run attempts** — defence in depth, and
  the only half that helps if the factor stays high. `approve <ID>`
  already resolves to one request and is O(1) in directory size, so a
  refusal has somewhere to send the user. A run that would exceed the
  bound refuses and names the ID form rather than working for an
  unbounded time.

**Recommendation: do both**, with the bound generous enough that
legitimate use never meets it. **Deliberately left `[ ]`** because the
first is a security parameter and this document's convention is that
those get reviewed rather than settled in passing — and because the
cheap, obvious-looking move (leave the factor at 19, it is "more
secure") is the one that keeps the DoS and slows every honest approval
down, which is worth someone actually deciding rather than defaulting
into.
