...
- [x] An exact title match resolves uniquely even when it's also a
      substring of another entry's title
- [x] A unique (non-exact) substring title match resolves correctly
- [x] An exact UUID resolves to the correct entry, including the
...
- [x] A unique UUID substring (not just a prefix, and allowed to span a
      hyphen) resolves to the correct entry
- [x] **An exact title match wins over a UUID match, even when the query
...
- [x] An ambiguous substring match (title or UUID) lists all candidates
      and, in one-shot mode, fails with nonzero exit instead of prompting
- [x] The resolver returns a candidate list as a *value*; no library code
      path prints it
- [x] A query matching nothing is distinguishable from a query matching
      several — different typed results, different exit codes
- [x] `gage cat` and `gage rm` resolve every stage (exact/substring
      title, exact/substring UUID) exactly like `show` — no longer
      limited to M4's UUID-or-exact-title-only matching
- [x] An ambiguous query given to `cat` or `rm` lists candidates and fails
...
- [x] `gage show` prints only the `value` field — not title, description,
      fields, or timestamps
- [x] `gage cat` prints the full decrypted YAML including metadata,
      distinguishing it from `show`
- [x] `gage edit` re-stamps `updated`/`updated_by` on save, leaves other
      fields untouched if unedited, and produces a new commit
- [x] `gage edit` that comes back with unparseable YAML aborts with no
      commit and a clear error, and the original entry is unchanged
- [x] `gage rename` changes only the title (not `value`/`fields`) and
      bumps `updated`
- [x] `gage rename` to an existing title behaves per the decision above
- [x] `gage generate` inserts a new entry with a randomly generated
      `value` of a sane default length — `-l`/`--no-symbols` customization
      is deferred to M12, tested there
- [x] `gage generate` draws from a cryptographically secure source
      (`crypto/rand`), asserted structurally rather than statistically

**`insert -e` and the shared editor helper**

- [x] `gage insert -e` opens a template (`title`/`description`
...
- [x] `gage insert -e` aborts with no entry written and no commit if the
      file comes back unchanged, or with `value` still empty and `fields`
      still empty
- [x] `gage insert -e` changing `title` inside the editor uses the
      edited title, not the original `<title>` argument, for both the
      saved entry and the duplicate-title/`-f` check
- [x] `-e` is rejected alongside `-m` or `--value-stdin`, before any I/O
      (extends M4's mutual-exclusion test to three flags)
- [x] `editYAML`'s scratch file lands in a verified tmpfs directory on
      Linux and the OS's standard temp directory elsewhere, created
      `0600`
- [x] The scratch file is removed (contents overwritten first) on every
      exit path — `$EDITOR` exits zero, `$EDITOR` exits non-zero, and the
      edited content fails to parse back into an `Entry`
- [x] An unset or unusable `$EDITOR` fails with a clear error before a
      scratch file containing plaintext is written

## Implementation

- [x] Query resolver (exact title/substring title/exact UUID/substring
...
- [x] `gage cat`/`gage rm` re-wired onto the shared resolver, replacing
      M4's exact-UUID/title-only matching
- [x] `gage show` (value only, `--field` deferred to M12)
- [x] Cross-platform scratch-file location for `editYAML`: on Linux,
      ...
      exclusively created)
- [x] Shared `editYAML` CLI helper: write the scratch file, launch
      ...
      `gage insert -e`
- [x] `gage edit` (`editYAML` seeded with the decrypted entry, re-stamps
      `updated`/`updated_by`)
- [x] `gage insert -e` (`editYAML` seeded with a stub entry instead of a
      decrypted one)
- [x] `gage rename`
- [x] `gage generate` (default-length/character-set value only, from
