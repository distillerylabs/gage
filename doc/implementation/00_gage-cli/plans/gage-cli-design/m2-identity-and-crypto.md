...
- [x] Encrypt → decrypt round-trip recovers a byte-identical payload
- [x] A payload encrypted to N recipients is independently decryptable by
      each recipient's private key
- [x] Decrypting with a key that is *not* a recipient fails with a clear
      typed error, not a panic or silent garbage output
- [x] Corrupted/truncated ciphertext fails decryption cleanly
- [x] Encrypting to zero recipients is refused rather than producing a
      file nobody can read
- [x] Attempting to combine a scrypt passphrase recipient with any other
      recipient is refused — the invariant the identity file relies on
      (see [A3](open-questions.md))

**Identity generation & unlock**

- [x] `gage init` (no `--recipient`) now succeeds, generating the
      device's identity and writing its public key to both recipient
      files — replaces M1's "fails without `--recipient`" test
- [x] `gage init --recipient <extra>` writes both the generated device
      key and the extra key, in that order
- [x] `gage init` writes the device's wrapped identity file to
      `$GAGE_DATA/identities/<vault>/<device>.age`, directory `0700` and
      file `0600`
- [x] The public key derived from the generated identity matches exactly
      what's written to both `.age-recipients` and `.gage/config.toml`'s
      `[[recipients]]`
- [x] `gage init` records this device's `device` and `method` under
      `[vaults.<name>]` in global config, and `Unlock` resolves the
      identity file from the former
- [x] `gage init --device NAME` uses that name for the identity file
      path, the `[[recipients]].device` label, and the global config
      record — all three agree
- [x] **No path is constructed from an unvalidated device name**: a
      ...
      (Q-DEVICE-NAME)
- [x] `Unlock` dispatches on **this device's** locally-recorded method,
...
- [x] Passphrase-method identity generation: fresh X25519 keypair,
...
- [x] Device-name and per-device-method resolution and recording, per
...
- [x] `Vault.Unlock(Prompter) (Identity, error)`: decrypts the wrapped
...
- [x] `cmd/gage`'s wrong-passphrase retry policy, per the decision above —
      in the CLI layer, never in the library
- [x] `internal/gage/memlock`: `Lock([]byte) error`/`Unlock([]byte) error`,
...
- [x] `Locker` interface between `Vault.Unlock`/`Identity.Close` and the
...
- [x] `Identity.Close()`: releases the page lock and zeroes the private
      key, cross-platform, via the same `memlock` package; idempotent
- [x] Process-wide core dump disabling at `cmd/gage` startup:
      ...
      parity)
- [x] `gage init` completed: default path generates the local identity;
