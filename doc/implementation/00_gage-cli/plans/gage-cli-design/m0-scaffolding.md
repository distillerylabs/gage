...
- [x] `make build` produces a `gage` binary
- [x] `make test` runs with no network access and no pre-existing keys
- [x] `make lint` and `make fmt --check` exit 0 on a clean tree
- [x] a lint check fails if `internal/gage` (the library layer) references
      `os.Stdin`/`os.Stdout` or calls `fmt.Print*`/`os.Exit` outside
      `_test.go` files — keeps CLI-only I/O out of the library from day one
- [x] `gage --help` exits 0 and lists the registered top-level subcommands
- [x] `gage help` exits 0 and produces output identical to `gage --help` —
      the spelling every operator tries first
- [x] `gage help <subcommand>` prints that subcommand's usage and exits 0;
      `gage help <unknown>` fails with the usage exit code
- [x] A Cobra-native usage rejection (wrong argument count, unknown flag)
...
- [x] A command's own coded error (exit code already set via the
...
- [x] Neither help spelling lists the session-only commands
...
- [x] `gage --version` prints a version string including the build's
      commit and, on a tagged build, the tag
- [x] Bare `gage` with a TTY on stdin enters session mode rather than
      printing help (Q-ROOT-CMD) — asserted at the dispatch level in M0;
      the session itself is M6's
- [x] Bare `gage` with stdin piped or redirected prints help instead of
      entering a session, leaving `--stdin` as the one spelling for
      "read session commands from stdin"
- [x] XDG path resolution: correct config/data/state paths on Linux/macOS
      with `XDG_*` env vars set, and with them unset (defaults)
- [x] XDG path resolution: correct `%APPDATA%`/`%LOCALAPPDATA%`-based paths
      on Windows
- [x] Global `config.toml` round-trip: write a `vaults.*`/`current` struct,
      read it back, fields match exactly
- [x] An interrupted global-config write (simulated failure between temp
      write and rename) leaves the previous `config.toml` intact and
      parseable — never a truncated file
- [x] Local identity path resolves to `$GAGE_DATA/identities/<vault>/<device>.age`
      — a sibling of `$GAGE_DATA/vaults/`, never nested inside a vault's own
      directory
- [x] `vaultlock`: two acquisitions of the same lock path from
      independent processes are serialized — the second observes the
      first's hold rather than proceeding concurrently
- [x] `vaultlock`: releasing lets the waiter proceed; a process exiting
      while holding the lock releases it (no stale lock survives a kill)
- [x] `vaultlock` round-trips on the current platform, Linux/macOS
      (`flock`) and Windows (`LockFileEx`) alike
- [x] Every defined exit code is produced by at least one code path, and
      no command returns a bare `1` that isn't in the taxonomy
- [x] `gittest.NewBareRemote` (or similar) creates an ephemeral local
      bare repo under `t.TempDir()` and returns its path; two calls
      produce independent, non-colliding repos
- [x] CI runs `make build`/`make test`/`make lint` on Linux, macOS, *and*
...
- [x] `go.mod` + directory layout: `cmd/gage` (thin Cobra frontend) and
      `internal/gage` (library — vault/session/entry logic, no terminal
      I/O), split from the start so later milestones build into the
      right layer
- [x] `Makefile` with `build`, `test`, `lint`, `fmt`, `clean` targets
- [x] `.gitignore` (binaries, `dist/`, etc.)
- [x] CI workflow with a Linux/macOS/Windows runner matrix invoking
...
- [x] Cobra root command + subcommand dispatch skeleton; `--version` with
...
- [x] **Command registry** in `cmd/gage` (not `internal/gage` — command
...
- [x] `gage help` and `gage help <subcommand>` wired as equivalents to
      `gage --help` / `<subcommand> --help`
- [x] Availability tagging for the commands that exist by the end of M0
...
- [x] Exit-code taxonomy as a single library-side enum, rendered to
      process exit codes by `cmd/gage` (the library itself never calls
      `os.Exit`)
- [x] skeleton `Prompter` (or similar) callback interface in
      ...
      interface change
- [x] skeleton `Identity` type (wraps unlocked private key material) with
      ...
      reworking M4/M5
- [x] `internal/gage/vaultlock`: cross-platform advisory file lock —
      ...
      first caller
- [x] Atomic TOML write helper (write temp in the same directory, fsync,
      ...
      registered vault
- [x] XDG path resolution (config/data/state, with Windows mapping)
- [x] Local identity path helper: `$GAGE_DATA/identities/<vault>/<device>.age`
      (directory `0700`, file `0600`)
- [x] Global `$GAGE_CONFIG/config.toml` read/write
- [x] Test harness: in-process `rootCmd.Execute()` runner with injected
...
- [x] `internal/gage/gittest` test-helper package: `NewBareRemote(t)
