...
func planRecoveryKey(app *App, noRecoveryKey bool, outPath string) (recoveryKeyMode, error) {
	...
	if outPath != "" {
		if err := checkRecoveryKeyOutPath(outPath); err != nil {
			return recoveryKeyNone, err
		}
...
// should not: an existing file is never overwritten, because the realistic
// thing at a path named "recovery.key" is another vault's recovery key.
func checkRecoveryKeyOutPath(path string) error {
