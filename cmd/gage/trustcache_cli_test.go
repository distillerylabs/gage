...
// TestReadsWarnOpportunisticallyInOneShotMode is the CLI face of the
// plan's "a non-blocking opportunistic warning fires on a plain `gage
// show`/`ls` in one-shot mode when recipients have changed, even though
// neither command encrypts anything."
//
// The library test for this asserts the hook is on Vault.Unlock; what is
// only checkable here is that it actually reaches the operator — on
// stderr, so a redirected stdout still carries nothing but the entry —
// and that it stays advisory: both commands succeed, neither asks, and
// the cache is not regenerated by a read.
func TestReadsWarnOpportunisticallyInOneShotMode(t *testing.T) {
	isolateXDG(t)
	vaultPath := initVaultForTest(t, "personal", "--device", "laptop-1")
	bootstrapTrustCache(t)

	if res, _ := runCLIWithValue(t, []string{"insert", "ProtonMail"}, "hunter2"); res.Code != 0 {
		t.Fatalf("seeding an entry: %d %s", res.Code, res.Stderr)
	}
	commitRoutineRecipientChangeCLI(t, vaultPath, "phone-1", newRecipientKey(t))

	cacheBefore, err := os.ReadFile(filepath.Join(trustCacheDirForTest(t, "personal"), "known-config.toml"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		{"ls", []string{"ls"}},
		{"show", []string{"show", "ProtonMail"}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			// A plain fakePrompter, not the declining one: only
			// *fakePrompter gets its warnings echoed to the run's stderr
			// (see runCLIWithPrompterAndStdin), and "the operator
			// actually sees this" is half of what this test is about.
			// Its ConfirmRecipientChange still records every question,
			// which is the other half.
			p := &fakePrompter{passphrases: []string{testPassphrase}}
			res, _ := runCLIWithPrompter(t, tc.args, "", false, p)
			if res.Code != 0 {
				t.Fatalf("%s exit code = %d, want 0; a read must not be blocked by the advisory: %s",
					tc.name, res.Code, res.Stderr)
			}
			if !strings.Contains(res.Stderr, "recipient list changed") {
				t.Errorf("`gage %s` did not surface the opportunistic warning:\nstderr=%q", tc.name, res.Stderr)
			}
			if strings.Contains(res.Stdout, "recipient list changed") {
				t.Errorf("the advisory went to stdout, which carries the entry: %q", res.Stdout)
			}
			if len(p.recipientChanges) != 0 {
				t.Errorf("`gage %s` asked for a decision %d times; the opportunistic check never blocks",
					tc.name, len(p.recipientChanges))
			}
		})
	}

	// A read must not launder an unreviewed list into an approved one.
	cacheAfter, err := os.ReadFile(filepath.Join(trustCacheDirForTest(t, "personal"), "known-config.toml"))
	if err != nil {
		t.Fatal(err)
	}
	if string(cacheAfter) != string(cacheBefore) {
		t.Error("a read regenerated the trust cache; only a confirmed change may do that")
	}
}

// TestSessionModeAsksTheSameRecipientQuestion is the half of the plan's
// "the warning fires the same way for a one-shot insert/edit/generate as
// for a session-mode one" that the library test cannot reach: it asserts
// the *absence* of a Session, and a session is the case it is contrasted
// with.
//
// `rename` is the verb because it encrypts without a value prompt, so
// the transcript below has exactly one question in it and the `n` on the
// third line can only be answering the recipient change.
func TestSessionModeAsksTheSameRecipientQuestion(t *testing.T) {
	isolateXDG(t)
	vaultPath := initVaultForTest(t, "personal", "--device", "laptop-1")
	if res, _ := runCLIWithValue(t, []string{"insert", "ProtonMail"}, "hunter2"); res.Code != 0 {
		t.Fatalf("seeding an entry: %d %s", res.Code, res.Stderr)
	...
	res := runSessionScript(t, script(
		"use personal",
		testPassphrase,
		`rename ProtonMail "Proton Mail"`,
		"n",
		"exit",
	))

	// The real terminalPrompter rendered the real question, inside a
	// session, from the same Vault.Rename hook a one-shot run uses. A
	// session repoints the prompter at app.Out and reads through the
	// line editor (see useSessionReader), so the transcript is both
	// streams together rather than stderr alone.
	transcript := res.Stdout + res.Stderr
	for _, want := range []string{"Recipients for", "phone-1", "[y/N]"} {
		if !strings.Contains(transcript, want) {
			t.Errorf("the session's recipient prompt is missing %q:\n%s", want, transcript)
		}
	}
	// And declining inside a session blocks, exactly as it does outside.
	if out := runCLI(t, []string{"ls"}, ""); !strings.Contains(out.Stdout, "ProtonMail") ||
		strings.Contains(out.Stdout, "Proton Mail") {
		t.Errorf("the rename went through despite the refusal:\n%s", out.Stdout)
	}
}
